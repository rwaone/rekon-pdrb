...
function getSummarise(type) {
    ...
    if (type === "lapangan-usaha") {
        ...
        for (let q = 1; q <= 16; q++) {
            ...
            $(`#sector-1-${q}`).text(
                formatRupiah(String(sum).replaceAll(/[.]/g, ","))
            );
            sum = sum - sum;
        }
        sum = sum - sum;
        ...
        for (let q = 1; q <= 16; q++) {
            let pdrb = calculateSector(`pdrb-total-${q}`);
            let nonmigas =
        ...
        for (let q = 1; q <= 16; q++) {
            ...
            $(`#sector-13-${q}`).text(
                formatRupiah(String(Nets).replaceAll(/[.]/g, ","))
            );
...
function getGrowth(data, before, type) {
    ...
    for (let i = 0; i < adhk_now.length; i++) {
        let score =
            // adhk_before[i] > 0
            //     ? ((adhk_now[i] / adhk_before[i]) * 100 - 100)
            //     : "-";
            ((adhk_now[i] / adhk_before[i]) * 100 - 100);
        growth.push(score);
    }
